...
async def stream_to_speakers(text: str, stream_id: int) -> None:
    ...
    try:
        async with openai.audio.speech.with_streaming_response.create(
            model="kokoro", voice="af_bella", response_format="pcm", input=text
        ) as response:
            print(
                f"Stream {stream_id} - Time to first byte: {int((time.time() - start_time) * 1000)}ms"
            )
        ...
        print(
            f"Stream {stream_id} completed in {int((time.time() - start_time) * 1000)}ms"
        )
...
async def save_to_file(text: str, file_id: int) -> None:
    ...
    async with openai.audio.speech.with_streaming_response.create(
        model="kokoro", voice="af_bella", input=text
    ) as response:
...
async def main() -> None:
    ...
    speaker_tasks = [
        stream_to_speakers(text, i) for i, text in enumerate(texts)
    ]

    # Create tasks for saving to files
    file_tasks = [save_to_file(text, i) for i, text in enumerate(texts)]

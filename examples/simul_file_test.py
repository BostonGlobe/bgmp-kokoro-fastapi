...
async def save_to_file(text: str, file_id: int) -> None:
    ...
    try:
        ...
        async with openai.audio.speech.with_streaming_response.create(
            model="kokoro", voice="af_bella", input=text, response_format="mp3"
        ) as response:
            print(
                f"File {file_id} - Time to first byte: {int((time.time() - start_time) * 1000)}ms"
            )
            ...
            print(
                f"File {file_id} completed in {int((time.time() - start_time) * 1000)}ms"
            )

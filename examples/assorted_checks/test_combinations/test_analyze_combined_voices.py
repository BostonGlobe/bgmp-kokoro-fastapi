...
def plot_comparison(analyses, output_dir):
    ...
    ax = plt.subplot(111)
    ax.bar(
        x - width, rms_values, width, label="RMS (Texture)", color="#ff2a6d"
    )
    ax.bar(
        x,
        [c / 1000 for c in centroids],
        width,
        label="Spectral Centroid/1000 (Brightness)",
        color="#05d9e8",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "characteristics.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    voices = {
        ...
        "af_bella(0.2)+af_kore(0.8)": output_dir
        / "af_bella_20_af_kore_80.wav",
        "af_bella(0.8)+af_kore(0.2)": output_dir
        / "af_bella_80_af_kore_20.wav",
        "af_bella(0.5)+af_kore(0.5)": output_dir
        / "af_bella_50_af_kore_50.wav",
    }

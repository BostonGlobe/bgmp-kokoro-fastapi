...
def trim_voice_tensor(tensor):
    ...
    if tensor.shape[0] != 511:
        raise ValueError(
            f"Expected tensor with first dimension 511, got {tensor.shape[0]}"
        )
...
def process_voice_file(file_path):
    ...
    try:
        ...
        if tensor.shape[0] != 511:
            logger.info(
                f"Skipping {os.path.basename(file_path)} - already correct shape {tensor.shape}"
            )
            return False
...
def main():
    ...
    logger.info(
        "To restore backups if needed, remove .backup extension to replace trimmed files"
    )

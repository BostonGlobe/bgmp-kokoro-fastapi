...
def load_longform(n_lines: int) -> str:
    lines = [
        ln.strip() for ln in open(TEXT_FILE, encoding="utf-8") if ln.strip()
    ]
    return " ".join(lines[:n_lines])


def speech(
    base_url: str, text: str, voice: str, fmt: str
) -> tuple[bytes, float]:
...
def run_scenario(label, text, args) -> dict:
    ...
    def mark(name):
        events.append(
            {"time": time.time() - monitor.start_time, "label": name}
        )
    ...
    alen = (
        get_audio_length(audio1, audio_dir) if args.format == "wav" else None
    )
    ...
    print(
        f"  -> reclaimed {stats['reclaimed_mib']} MiB on unload; "
        f"reload cost +{stats['reload_penalty_seconds']}s"
    )

    plot_path = os.path.join(
        SCRIPT_DIR, "output_plots", f"model_unload_{label}.png"
    )
...
def plot_timeline(df, events, stats, loaded, floor, output_path, label):
    ...
    ax.axhline(
        ...
        alpha=0.55,
        linewidth=1.5,
        label=f"unloaded floor  {floor:.2f} GB",
    )
    ax.fill_between(
        t, floor, loaded, color=STYLE_CONFIG["secondary_color"], alpha=0.08
    )
    ax.annotate(
        f"reclaimed {stats['reclaimed_mib']:.0f} MiB",
        xy=(0.5, (loaded + floor) / 2),
    ...
    setup_plot(
        fig,
        ax,
        f"Model Unload / Reload VRAM ({label})",
        xlabel="Time (seconds)",
...
def replot_from_saved():
    ...
    for r in data["scenarios"]:
        ...
        stats = r["stats"]
        plot_path = os.path.join(
            SCRIPT_DIR, "output_plots", f"model_unload_{r['label']}.png"
        )
        plot_timeline(
            df,
            r["events"],
...
def main():
    ...
    ap.add_argument(
        "--format", default="wav", help="wav enables audio-length/RTF"
    )
    ap.add_argument(
        "--long-lines",
        type=int,
    ...
    write_benchmark_stats(
        [
            {"title": f"Model Unload - {r['label']}", "stats": r["stats"]}
            for r in results
        ],

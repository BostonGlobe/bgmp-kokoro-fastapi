...
def plot_results(results: dict):
    ...
    non_streaming_times = [
        r["non_streaming"]["total_time"] for r in results.values()
    ]
    first_byte_times = [
        r["streaming"]["first_byte_time"] for r in results.values()
    ]
...
async def main():
    ...
    for name, text in TEST_TEXTS.items():
        ...
        first_byte_time, stream_total_time, stream_bytes = (
            await benchmark_streaming(name, text)
        )

        # Test non-streaming
        print("Running non-streaming test...")
        non_stream_total_time, non_stream_bytes = (
            await benchmark_non_streaming(name, text)
        )
        ...
        results[name] = {
            ...
            "non_streaming": {
                ...
                "throughput": non_stream_bytes
                / non_stream_total_time
                / 1024,  # KB/s
            },
        }
        ...
        print(
            f"  Throughput: {non_stream_bytes/non_stream_total_time/1024:.1f} KB/s"
        )
